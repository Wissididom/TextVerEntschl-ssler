...
Public Class Blowfish
    ...
    Public Event Terminated(ErrorMessage As String)
    Public Event Complete()

    Private KeyStore As Byte()
    Private LastError As String
    Private WithEvents SessionFish As BlowfishX

    Public Sub New()
        UpdateKey(StringToBytes("DEFAULTKEYVALUE"))
    End Sub

    Public Sub New(Key As String)
        UpdateKey(StringToBytes(Key))
    End Sub
    ...
    Public WriteOnly Property Key As Byte()
        Set(value As Byte())
            UpdateKey(value)
        End Set
    End Property

    Public Function GetEncodedLength(ByteLength As Long) As Long
        Return (CInt((ByteLength + 3) \ 8) * 8) + 16
    End Function

    Public Function EncodeBytes(Bytes As Byte()) As Byte()
        Try
            PrepareTheFish()
            Return SessionFish.Encode(Bytes)
    ...
    Private Sub UpdateKey(Key As Byte())
        If Key.Length > 56 Then
            LastError = DateTime.Now & " : " & "Key cannot exceed 56 bytes"
            Throw New Exception(LastError)
        End If
        If IsNothing(Key) OrElse Key.Length = 0 Then
            LastError = DateTime.Now & " : " & "Key cannot be null or 0 length"
            Throw New Exception(LastError)
        Else
            KeyStore = Key
        End If
    ...
    Private Sub PrepareTheFish()
        SessionFish = New BlowfishX(KeyStore)
        AddHandler SessionFish.Complete, AddressOf ProcessComplete
        AddHandler SessionFish.Progress, AddressOf ProcessProgress
    End Sub

    Private Function ProcessProgress() As Object
    ...
    Private Sub ProcessProgress(Percent As Integer)
        RaiseEvent Progress(Percent)
    End Sub

    Private Function StringToBytes(StringValue As String) As Byte()
        Return System.Text.Encoding.UTF8.GetBytes(StringValue)
    End Function

    Private Class BlowfishX
        Private Const ROUNDS = 16
        Private m_pBox(0 To ROUNDS + 1) As Long
        Private m_sBox(0 To 3, 0 To 255) As Long
        Private m_ByteKeyValue As Byte()
        Private m_RunningCompiled As Boolean

        Public Event Progress(ByVal PercentComplete As Integer)
        Public Event Complete()

        Private WriteOnly Property Key As Byte()
            Set(New_Value As Byte())
                Initialize()
                Dim i As Long
                Dim j As Long
                Dim k As Long
                ...
                Dim KeyLen As Integer = New_Value.Length
                'Do nothing if the key is buffered
                If Not IsNothing(m_ByteKeyValue) AndAlso New_Value.Length = m_ByteKeyValue.Length Then
                    Result = New_Value.SequenceEqual(m_ByteKeyValue)
                Else
                    Result = False
                End If
                'End - Do nothing if the key is buffered
                If Result = True Then Exit Property
                'Convert the new key into a bytearray
                ReDim m_ByteKeyValue(New_Value.Length - 1)
                Buffer.BlockCopy(New_Value, 0, m_ByteKeyValue, 0, KeyLen)
                KeyLength = New_Value.Length
                Key = New_Value
                'Create key-dependant p-boxes
                For i = 0 To (ROUNDS + 1)
                    dataX = 0
                    For k = 0 To 3
                        dataX = (dataX << 8) Or Key(j)
                        j = (j + 1) Mod KeyLength
                    Next
        ...
        Public Function Encode(Bytes As Byte()) As Byte()
            Dim ByteArray(Bytes.Length - 1) As Byte
            Buffer.BlockCopy(Bytes, 9, ByteArray, 0, Bytes.Length)
            Dim Offset As Long
            Dim OrigLen As Long
            ...
            Buffer.BlockCopy(ByteArray, 0, ByteArray, 12, OrigLen)
            'Store the length descriptor in bytes [9-12]
            CopyMem(ByteArray, 8, OrigLen, 4)
            ...
            'same key)
            CopyMem(ByteArray, 0, CLng(2147483647 * Rnd()), 4)
            CopyMem(ByteArray, 4, CLng(2147483647 * Rnd()), 4)
        ...
        Public Function Decode(Bytes As Byte()) As Byte()
            Dim ByteArray(Bytes.Length - 1) As Byte
            ...
            Next
            'Get the size of the original array
            'Call CopyLBMem(OrigLen, ByteArray(8), 4)
            CopyMem(OrigLen, ByteArray, 8, 4)
            ...
            If (CipherLen - OrigLen > 19) Or (CipherLen - OrigLen < 12) Then
                Throw New Exception("Unable To Decode. Data may not be encoded or an incorrect key was used.")
                Return Nothing
            End If
            'Resize the bytearray to hold only the plaintext
            'and not the extra information added by the
            'encryption routine (The Random Bytes)
        ...
        Private Function F(ByVal x As Long) As Long
        ...
        Private Sub DecryptBlock(ByRef Xl As Long, ByRef Xr As Long)
            Dim i As Long
            Dim j As Long
            Dim Temp As Long
            Temp = Xr
        ...
        Private Sub EncryptBlock(ByRef Xl As Long, ByRef Xr As Long)
            Dim i As Long
            Dim j As Long
            Dim Temp As Long
            j = 0
        ...
        Private Sub CopyMem(ByRef Bytes As Byte(), offset As Long, Lng As Long, Length As Long)
            Dim MyGC As GCHandle = GCHandle.Alloc(Lng, GCHandleType.Pinned)
            Dim AddofLongValue As IntPtr = MyGC.AddrOfPinnedObject()
            Marshal.Copy(AddofLongValue, Bytes, offset, Length)
            Lng = Marshal.ReadInt32(AddofLongValue)
            MyGC.Free()
        End Sub

        'Replaces VB6:
        'Private Declare Sub CopyMem Lib "kernel32" Alias "RtlMoveMemory"
        Private Sub CopyMem(ByRef Lng As Long, Bytes As Byte(), offset As Long, Length As Long)
            Dim MyGC As GCHandle = GCHandle.Alloc(Lng, GCHandleType.Pinned)
            Dim AddofLongValue As IntPtr = MyGC.AddrOfPinnedObject()
            Marshal.Copy(Bytes, offset, AddofLongValue, Length)
            Lng = Marshal.ReadInt32(AddofLongValue)
            MyGC.Free()
        End Sub

        Private Sub GetWord(ByRef LongValue As Long, CryptBuffer() As Byte, Offset As Long)
            Dim bb(0 To 3) As Byte
            bb(3) = CryptBuffer(Offset)
            bb(2) = CryptBuffer(Offset + 1)
            bb(1) = CryptBuffer(Offset + 2)
            bb(0) = CryptBuffer(Offset + 3)
            CopyMem(LongValue, bb, 0, 4)
        End Sub

        Private Sub PutWord(LongValue As Long, ByRef CryptBuffer() As Byte, Offset As Long)
            Dim bb(0 To 3) As Byte
            CopyMem(bb, 0, LongValue, 4)
        ...
        Private Function UnsignedAdd(ByVal Data1 As Long, Data2 As Long) As Long
            ...
            Dim Value As Long
            Dim a As Long
            CopyMem(x1, 0, Data1, 4)
            CopyMem(x2, 0, Data2, 4)
            Rest = 0
            For a = 0 To 3
